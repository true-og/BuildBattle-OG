...
public class Plot {
    ...
    public Plot(BaseArena arena, Biome biome) {
        ...
        defaultFloor = XMaterial.matchXMaterial(
                arena.getPlugin().getConfig().getString("Floor.Material", "LOG").toUpperCase(java.util.Locale.ENGLISH))
    ...
    public boolean addMember(Player player, BaseArena playerArena, boolean silent) {
        ...
        if (members.contains(player)) {
            ...
            if (!silent)
                new MessageBuilder("IN_GAME_MESSAGES_PLOT_SELECTOR_MEMBER").asKey().arena(arena).player(player)
                        .sendPlayer();
            return false;
        ...
        if (members.size() >= arena.getArenaOption("PLOT_MEMBER_SIZE")) {
            ...
            if (!silent)
                new MessageBuilder("IN_GAME_MESSAGES_PLOT_SELECTOR_FULL").asKey().prefix().arena(arena).player(player)
                        .sendPlayer();
            return false;
    ...
    public void resetPlot() {
        ...
        if (centerWorld != null) {
            ...
            for (Entity entity : centerWorld.getEntities()) {
                ...
                if (cuboid.isInWithMarge(entity.getLocation(), 5)) {
                    ...
                    if (isCitizensEnabled && CitizensAPI.getNPCRegistry() != null
        ...
        if (centerWorld != null) {
            ...
            if (ServerVersion.Version.isCurrentHigher(ServerVersion.Version.v1_15)) {
                ...
                centerWorld.setBiome(min.getBlockX(), Math.min(min.getBlockY(), max.getBlockY()), max.getBlockZ(),
                        plotDefaultBiome);

            } else {

                centerWorld.setBiome(cuboid.getMinPoint().getBlockX(), cuboid.getMaxPoint().getBlockZ(),
                        plotDefaultBiome);
