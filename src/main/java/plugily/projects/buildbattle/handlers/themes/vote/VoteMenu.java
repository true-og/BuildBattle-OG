...
public class VoteMenu {
    ...
    private NormalFastInv getGUI(Player guiPlayer) {
        ...
        NormalFastInv gui = new NormalFastInv(9 * themeSelection.size(),
                new MessageBuilder("MENU_THEME_INVENTORY").asKey().build());
        ...
        for (String theme : themeSelection) {
            ...
            gui.setItem(multiplied,
                    new SimpleClickableItem(
                            new ItemBuilder(new ItemStack(XMaterial.OAK_SIGN.parseMaterial()))
                                    .name(new MessageBuilder("MENU_THEME_ITEM_NAME").asKey().arena(arena).value(theme)
                                            .integer((int) percent).build())
                                    .lore(new MessageBuilder("MENU_THEME_ITEM_LORE").asKey().arena(arena).value(theme)
                                            .integer((int) percent).build().split(";"))
                                    .build(),
                            getSignClickEvent(theme)));

            gui.setItem(multiplied + 1, ClickableItem
                    .of(new ItemBuilder(XMaterial.IRON_BARS.parseItem()).name("&7->").colorizeItem().build()));

            setGlassPanes(gui, multiplied, percent);

            if (plugin.getConfigPreferences().getOption("SUPER_VOTES")) {

                gui.setItem(multiplied + 8,
                        new SimpleClickableItem(new ItemBuilder(new ItemStack(Material.PAPER))
                                .name(new MessageBuilder("MENU_THEME_VOTE_SUPER_ITEM_NAME").asKey().arena(arena)
                                        .value(theme).build())
                                .lore(new MessageBuilder("MENU_THEME_VOTE_SUPER_ITEM_LORE").asKey().player(guiPlayer)
                                        .arena(arena).value(theme).build().split(";"))
                                .build(), event ->
                        {
                                    ...
                                    if (user.getStatistic("SUPER_VOTES") > 0) {
                                        ...
                                        new MessageBuilder("MENU_THEME_VOTE_SUPER_USED").asKey().arena(arena)
                                                .player(player).value(theme).sendArena();
                ...
                gui.setItem(multiplied + 8, ClickableItem
                        .of(new ItemBuilder(XMaterial.IRON_BARS.parseItem()).name("&7<-").colorizeItem().build()));
    ...
    public void updatePlayerGui(Player player, NormalFastInv gui) {
        ...
        for (String theme : themeSelection) {
            ...
            itemBuilder.lore(new MessageBuilder("MENU_THEME_ITEM_LORE").asKey().arena(arena).value(theme)
                    .integer((int) percent).build().split(";"));
            ...
            gui.setItem(multiplied,
                    new SimpleClickableItem(itemBuilder.colorizeItem().build(), getSignClickEvent(theme)));
        ...
        for (int j = 0; j < 6; j++) {
            ...
            if (vote > percent) {
                ...
                gui.setItem(slot, ClickableItem
                        .of(new ItemBuilder(XMaterial.RED_STAINED_GLASS_PANE.parseItem()).name("???").build()));
                continue;

            }

            gui.setItem(slot, ClickableItem
                    .of(new ItemBuilder(XMaterial.LIME_STAINED_GLASS_PANE.parseItem()).name("???").build()));
            vote += 16.7;
